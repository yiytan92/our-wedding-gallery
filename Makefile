...
api/dev/start: api/dev/s3 api/dev/dynamodb
	@cd api \
	&& ${API_DEV_ENV_VARS} ./node_modules/.bin/serverless offline --region ap-southeast-1 --stage dev
...
api/dev/dynamodb:
	...
	&& ${API_DEV_ENV_VARS} ./node_modules/.bin/serverless dynamodb migrate --region ap-southeast-1 --stage dev
...
api/deploy: _require_AWS_ACCESS_KEY_ID _require_AWS_SECRET_ACCESS_KEY _require_STAGE
	@cd api \
	&& yarn \
	&& ./node_modules/.bin/serverless deploy --stage ${STAGE} --region ap-southeast-1 --verbose --conceal
...
client/deploy: _require_AWS_ACCESS_KEY_ID _require_AWS_SECRET_ACCESS_KEY _require_STAGE
	...
	&& docker run --rm \
	  ...
	  docker.io/amazon/aws-cli:2.4.6 \
	    bash -c "CLIENT_S3_BUCKET_NAME=\$$(aws ssm get-parameter --region=ap-southeast-1 --name /our-wedding/${STAGE}/apps/gallery/client-s3-bucket-name --query Parameter.Value --output text); \
	             aws s3 sync --region=ap-southeast-1 /build "s3://\$${CLIENT_S3_BUCKET_NAME}" --quiet --delete --sse AES256"
